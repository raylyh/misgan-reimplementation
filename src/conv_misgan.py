...
from generators_conv import convDataGenerator, convMaskGenerator
from discriminators_conv import convDiscriminator
...
# DCGAN tutorial on PyTorch - Weights initialisation
def weights_init(m):
...
G_data = convDataGenerator(latent_size=latent_size, features=features).to(device)
G_mask = convMaskGenerator(latent_size=latent_size, features=features, temperature=temperature).to(device)

D_data = convDiscriminator(features=features, batch_size=batch_size).to(device)
D_mask = convDiscriminator(features=features, batch_size=batch_size).to(device)
